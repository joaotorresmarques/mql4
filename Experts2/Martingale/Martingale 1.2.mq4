...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++FECHAR TODAS ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
    ...
    //fui dormir. será q ficaria legal um switch?

   NumOfTrades = CountTrades();
   //iLots = NormalizeDouble(Lots * MathPow(LotExponent, NumOfTrades), lotdecimal); 
     iLots = Lots+(NumOfTrades*1.0); //anda de 2 em 2. 0.01 - 0.03 - 0.05...
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ABERTURA ORDEM++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
   if(Time[0]>=tempo)
   {
      if(OrdersTotal()==0) 
      {
         if(Open[0]>MA) ticket = OrderSend(Symbol(),OP_SELL,iLots,Bid,0,0,(Bid-10*MyPoint)-Spread*Point,NULL,MagicNumber,0,Red);
         //if(Open[0]<MA) ticket = OrderSend(Symbol(),OP_BUY,iLots,Ask,0,0,(Ask+10*MyPoint)+Spread*Point,NULL,MagicNumber,0,Blue);
      }    
    }
   ...
   for (int trade=OrdersTotal()-1; trade>=0; trade--) 
   {
      if (OrderSelect(trade,SELECT_BY_POS, MODE_TRADES)) 
      {
         if(OrderSymbol()==Symbol() && OrderMagicNumber()==MagicNumber)
         {
            ...
            {
               if(Bid-5*MyPoint>OrderOpenPrice()) ticket = OrderSend(Symbol(),OP_SELL,iLots,Bid,0,0,(Bid-10*MyPoint)-Spread*Point,NULL,MagicNumber,0,Red);
               break; //(Bid-5*MyPoint)-Spread*Point
            }
            if(OrderType()==OP_BUY)
            {
               if(Ask+5*MyPoint<OrderOpenPrice()) ticket = OrderSend(Symbol(),OP_BUY,iLots,Ask,0,0,(Ask+10*MyPoint)+Spread*Point,NULL,MagicNumber,0,Blue);
               break;
           }      
         } 
      }     
