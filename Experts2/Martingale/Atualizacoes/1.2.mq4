...
/*=======================================================ANOTAÇÕES========================================================
  ...
  Otimo no 5M. 
  *Trallingstop
  *Aumento de lot quando tiver 4 ordens.
  *Aumento de take quando tiver 4 ordens.
========================================================================================================================*/
...
//=========================LOTES
      if(NumOfTrades==4) Lots = 0.05; if(NumOfTrades<4) Lots = 0.01;
      iLots = Lots+(NumOfTrades*0.02); //tava 0.02. 
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ABERTURA ORDEM++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
    if(Time[0]>=tempo+1000)
   {
      if(OrdersTotal()==0)
      { 
        if(Open[1]<Open[0]-15*MyPoint )  ticket = OrderSend(Symbol(),OP_BUY,iLots,Ask,0,0,0,Robo,MagicNumber,0,Blue);
        
       // if(Open[1]+15*MyPoint<Open[0] ) ticket = OrderSend(Symbol(),OP_SELL,iLots,Bid,0,0,0,Robo,MagicNumber,0,Red);
         ...
         if(OrderSymbol()==Symbol() && OrderMagicNumber()==MagicNumber)
         {
            ...
            if(OrderType()==OP_BUY)
            {
              if(Ask+STOP*MyPoint<OrderOpenPrice()) ticket = OrderSend(Symbol(),OP_BUY,iLots,Ask,0,0,0,NULL,MagicNumber,0,clrGreenYellow);
               break;                                                                           //Ask+(TAKE+NumOfTrades+4)*MyPoint 
           }//FIM OP_BUY
         ...
         if(OrderSymbol()==Symbol() && OrderMagicNumber()==MagicNumber)
         {
              ...
              if(NumOfTrades>3 )
            {
               if(OrderType()==OP_BUY)
               {
                  ...
                  if(SL2==0 && Ask-10*MyPoint>stnewpricebuy2) modify = OrderModify(OrderTicket(),OrderOpenPrice(),stnewpricebuy2+8*MyPoint,0,0,clrLightGreen);
                ...
                if(OrderType()==OP_SELL)
                  ...
                  if(SLsell2==0 && Bid+25*MyPoint<stnewpricesell2) modify = OrderModify(OrderTicket(),OrderOpenPrice(),stnewpricesell2-7*MyPoint,0,0,clrLightGreen);
                  
                  if(SLsell2>0 && Bid+10*MyPoint<SLsell2)   modify = OrderModify(OrderTicket(),OrderOpenPrice(),SLsell2-8*MyPoint,0,0,clrLightGreen);
