...
//=========================CONFIGURAÇÃO DE TAKES   
   ...
   if(count >= 10) takeSELL = TrallingStop*4;
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ABERTURA ORDEM++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
  if(TimeCurrent()>espera)
  {
    if((countbuy==0 && countbuystop==0) && (countsell==0 && countsellstop==0)  )
    {
         ...
         {
            ticket = OrderSend(Symbol(), OP_SELLLIMIT,Lots,Close[0]+DistancePips*MyPoint,Slippage,0,0,Robo,MagicNumber2,0,Red);
            ticket = OrderSend(Symbol(),OP_BUYLIMIT,Lots,Open[0]-DistancePips*MyPoint,Slippage,0,0,Robo,MagicNumber,0,Blue);
         } 
    }   
    ...
    if((countsell==0 && countsellstop==0) && (countbuy==0 && countbuystop==0))
    {
      ...
      {
         ticket = OrderSend(Symbol(), OP_SELLLIMIT,Lots,Open[0]+DistancePips*MyPoint,Slippage,0,0,Robo,MagicNumber2,0,Red);
            ticket = OrderSend(Symbol(),OP_BUYLIMIT,Lots,Close[0]-DistancePips*MyPoint,Slippage,0,0,Robo,MagicNumber,0,Blue);
