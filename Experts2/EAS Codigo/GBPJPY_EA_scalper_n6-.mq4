...
#define EAName "Expert Advisor Template"

extern string S1="---------------- Entry Settings";

extern int BBPeriod=50;
extern int Deviations1=2;
extern int Deviations2=3;
...
*/
   //|---------main signal
      ...
      double BBRU=iBands(NULL,0,BBPeriod,Deviations1,0,PRICE_CLOSE,MODE_UPPER,0);
      double BBRL=iBands(NULL,0,BBPeriod,Deviations1,0,PRICE_CLOSE,MODE_LOWER,0);
      double BBOU=iBands(NULL,0,BBPeriod,Deviations2,0,PRICE_CLOSE,MODE_UPPER,0);
      double BBOL=iBands(NULL,0,BBPeriod,Deviations2,0,PRICE_CLOSE,MODE_LOWER,0);
      double BBYU=iBands(NULL,0,BBPeriod,Deviations3,0,PRICE_CLOSE,MODE_UPPER,0);
      double BBYL=iBands(NULL,0,BBPeriod,Deviations3,0,PRICE_CLOSE,MODE_LOWER,0);
      
      double BBM=iMA(Symbol(),0,BBPeriod,0,MODE_SMA,PRICE_CLOSE,0);

      string BUY="false";
      string SELL="false";

      if(Bid<=((BBRL+BBOL)/2))BUY="true";
      if(Ask>=((BBOU+BBRU)/2))SELL="true";
...
//|---------close orders
   ...
   //if(Hedge==false&&Ask>=BBM)
   if(Hedge==false&&TakeProfit<=0)
   {
      if(ReverseSystem)CloseSellOrders(Magic);else CloseBuyOrders(Magic);
   }
   //if(Hedge==false&&Bid<=BBM)
   if(Hedge==false&&TakeProfit<=0)
   {
...
int CloseBuyOrders(int Magic)
{
  ...
  for (int cnt=total-1;cnt>=0;cnt--)
  {
    ...
    if(OrderMagicNumber()==Magic&&OrderSymbol()==Symbol())
    {
      if(OrderType()==OP_BUY)
      {
        if(Bid>=OrderOpenPrice()+(1*Point))OrderClose(OrderTicket(),OrderLots(),Bid,3);
      }
    }
  }
  return(0);
...
int CloseSellOrders(int Magic)
{
  ...
  for(int cnt=total-1;cnt>=0;cnt--)
  {
    ...
    if(OrderMagicNumber()==Magic&&OrderSymbol()==Symbol())
    {
      if(OrderType()==OP_SELL)
      {
        if(Ask<=OrderOpenPrice()-(1*Point))OrderClose(OrderTicket(),OrderLots(),Ask,3);
      }
    }
  }
  return(0);
