...
void OnTick()
  {     
double MA = iMA(Symbol(),NULL,MAperiod,0,MODE_SMA,PRICE_CLOSE,0);
//=========================POINT
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ABERTURA ORDEM++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
    if(Time[0]>=tempo+TimeWait)
   {
      if(OrdersTotal()==0)
      { 
        if(Open[0]<MA) ticket = OrderSend(Symbol(),OP_BUY,iLots,Ask,0,0,0,Robo,MagicNumber,0,Blue);
        
        if(Open[0]>MA) ticket = OrderSend(Symbol(),OP_SELL,iLots,Bid,0,0,0,Robo,MagicNumber,0,Red);
