...
6. Se o preço for dar STOP no MAGIC4 as outras ordens são finalizadas e ele fica aberto até dar TAKE.
...
/*=======================================================ANOTAÇÕES========================================================
FINALIZADO. Sistema 99,99% CLONE do KV S MOD.  (NÃO, NÃO ESTÁ)! KKKK. A MAGIC3 envia outra ordem antes de finalizar tudo.

Elementos que faltam para chegar na conta real:

   .Aprender e aplicar conceitos de SLIPPAGE
   .VPN
   .Testar em outras moedas. Funciona otimo no EUR/USD.
         ...
         if(OrdersTotal()==1)
         {
            ...
            for(cnt=0;cnt<total;cnt++)
            {
               ...
               if(OrderSymbol()==Symbol() && OrderMagicNumber()==MagicNumber1)
               {
                  ...
                  {  
                     ticket = OrderSend(Symbol(),OP_SELL,Lots2,Bid,0,0,0,NULL,MagicNumber2,0,Red);      
                  }//FIM OP_SELL   
         
                  if(OrderType()==OP_BUY)
                  {
                     ticket = OrderSend(Symbol(),OP_BUY,Lots2,Ask,0,0,0,NULL,MagicNumber2,0,Blue);
                  }//FIM OP_BUY
         ...
         if(OrdersTotal()==2 && existemagic4==false)
         {
            ...
            for(d=0;d<totald;d++)
            {
               ...
               if(OrderSymbol()==Symbol() && OrderMagicNumber()==MagicNumber2)
               {
                  ...
                  if(OrderType()==OP_SELL)
                  {  
                     if(STOP2shell==true) ticket = OrderSend(Symbol(),OP_SELL,Lots3,Bid,0,0,0,NULL,MagicNumber3,0,Red);
                   
                  }
               
                  if(OrderType()==OP_BUY)
                  {
                     if(STOP2buy==true) ticket = OrderSend(Symbol(),OP_BUY,Lots3,Ask,0,0,0,NULL,MagicNumber3,0,Blue);
                  }    
     ...
     if(OrdersTotal()==3  )
     {
      ...
      for(h=0;h<totalh;h++)
      {
         ...
         if(OrderSymbol()==Symbol() && OrderMagicNumber()==MagicNumber3)
         {
            if(OrderType()==OP_SELL)
            {
              if(STOP3shell ==true) ticket =  OrderSend(Symbol(),OP_BUY,Lots4,Ask,0,0,0,NULL,MagicNumber4,0,Blue);   
            }//FIM OP_SELL
            
            if(OrderType()==OP_BUY)
            {
              if(STOP3buy==true) ticket = OrderSend(Symbol(),OP_SELL,Lots4,Bid,0,0,0,NULL,MagicNumber4,0,Red);
            }//FIM OP_BUY
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++FECHAMENTO DE TODAS AS ORDENS++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++                     
     ...
     int v, totalv=OrdersTotal();
            for(v=0;v<totalv;v++)
            {
               ...
               {
                 if( TAKE4buy==true || TAKE4shell==true) ticket = OrderClose(OrderTicket(),OrderLots(),OrderClosePrice(),0,clrPink);
                } 
  ...
  int vt, totalvt=OrdersTotal();
            for(vt=0;vt<totalvt;vt++)
            {
               if(OrderSelect(vt,SELECT_BY_POS,MODE_TRADES))
               {
                  ...
                  {
                   if( TAKE2buy==true || TAKE2shell==true || TAKE3shell==true || TAKE3buy==true) ticket = OrderClose(OrderTicket(),OrderLots(),OrderClosePrice(),0,clrPink);
                } 
           }
            }//FIM CONTADOR  
