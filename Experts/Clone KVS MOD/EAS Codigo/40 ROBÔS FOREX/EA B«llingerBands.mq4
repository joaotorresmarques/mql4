...
string TradeComment = "_bolltrade_v03g.txt";
...
int start()
  {
   ...
   if(Close[0]>bup+(BDistance*Point)) 
     {
      SELLme=true; 
      // this happens at EVERY tick where Close>BDistance
      logwrite("BollTick3g.txt","---SELLme happened"); 
     }
   if(Close[0]<bdn-(BDistance*Point)) 
     {
      BUYme=true;  
      // this happens at EVERY tick where Close>BDistance
      logwrite("BollTick3g.txt","----BUYme happened"); 
     }

   // log excursions above upper and below lower bollinger bands (different filename)
   //int uDiff=((Close[0]-bup)/Point);
   //int dDiff=((bdn-Close[0])/Point);
   //if(Close[0]>bup) logwrite("BollTick3g.txt","Bdistance="+BDistance+" BUP="+bup+" Close[0]="+Close[0]+" Diff="+uDiff);
   //if(Close[0]<bdn) logwrite("BollTick3g.txt","Bdistance="+BDistance+" BDN="+bdn+" Close[0]="+Close[0]+" Diff="+dDiff);
