...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ABERTURA ORDEM++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++       
    if(OrdersTotal()==0)
    {
      ...
      if((High[0]-1*MyPoint == Open[0] || High[0]-1*MyPoint == Close[0]))
      {
            
            if(RSI>90) ticket = OrderSend(Symbol(),OP_BUY,Lots,Ask,Slippage,0,0,NULL,MagicNumber,0,Blue);
      }
      
      else if(High[0]-1*MyPoint ==Open[0] || High[0]-1*MyPoint == Close[0])
      {
            if(RSI<10)  ticket = OrderSend(Symbol(),OP_SELL,Lots,Bid,Slippage,0,0,NULL,MagicNumber,0,Red);
      }      
