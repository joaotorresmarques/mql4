...
int start() {
   ...
   if (Lock) {
      if (PriceOpenLastPos(Symbol(), OP_BUY, MagicNumber) - Ask > Lock_pips * GetPoint() && !ExistPositions(Symbol(), OP_SELL, MN_s)) {
         ...
         OpenPosition(Symbol(), OP_SELL, ld_136, ld_40, ld_48, MN_s, "лок бая");
      }
      if (Bid - PriceOpenLastPos(Symbol(), OP_SELL, MagicNumber) > Lock_pips * GetPoint() && !ExistPositions(Symbol(), OP_BUY, MN_b) && ExistPositions(Symbol(), OP_SELL, MagicNumber)) {
         ...
         OpenPosition(Symbol(), OP_BUY, ld_136, ld_40, ld_48, MN_b, "лок села");
      }
   }
   ...
   if (fNewBar_b() && !ExistPositions(Symbol(), OP_BUY, MagicNumber) && op_BUY) {
      if (ld_160 < pips) {
         ...
         OpenPosition(Symbol(), OP_BUY, l_lots_0, ld_40, ld_48, MagicNumber, "бай рабочий");
      }
   }
   if (fNewBar_s() && !ExistPositions(Symbol(), OP_SELL, MagicNumber) && op_SELL) {
      if (ld_160 > pips) {
         ...
         OpenPosition(Symbol(), OP_SELL, l_lots_0, ld_40, ld_48, MagicNumber, "селл рабочий");
      }
   }
   return (0);
}
...
void GetLogoType() {
   ...
   Label("ytg_logo_4", "И", 4, 373, 45, 26, "Wingdings 3", Red);
   Label("ytg_logo_5", "З", 4, 585, 9, 26, "Wingdings 3", Lime);
